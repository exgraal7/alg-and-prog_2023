...
Any::~Any() {
    if (type == Type::VECTOR_ANY_PTR) {
        for (auto element : *reinterpret_cast<std::vector<Any*>*>(data)) {
            delete element;
        }
    }
    delete data;
...
Any::operator std::vector<Any*>*() {
    ...
    return reinterpret_cast<std::vector<Any*>*>(data);
}

std::ostream& operator<<(std::ostream& out, const Any& val) {
    switch (val.type) {
        case Any::Type::INT:
            out << *reinterpret_cast<int*>(val.data);
            break;
        case Any::Type::DOUBLE:
            out << *reinterpret_cast<double*>(val.data);
            break;
        case Any::Type::STRING:
            out << *reinterpret_cast<std::string*>(val.data);
            break;
        case Any::Type::VECTOR_ANY_PTR: {
            auto vec = *reinterpret_cast<std::vector<Any*>*>(val.data);
